...
<fmt:message bundle="${loc}" key="local.administration" var="administration"/>
<fmt:message bundle="${loc}" key="local.locbutton.lang" var="lang" />
<fmt:message bundle="${loc}" key="local.locbutton.name.en" var="en" />
<fmt:message bundle="${loc}" key="local.locbutton.name.ru" var="ru" />
<fmt:message bundle="${loc}" key="local.locbutton.logout" var="logout" />
</head>
<body>
<body >
<jsp:useBean id="user" class="_java._ee._02._bean.User" scope="session"/>
      ...
      <div class="container">
        ...
        <div class="navbar-collapse collapse">
          <ul class="nav navbar-nav">
           <li><form action="Controller" method="get"><input type="hidden" name="command" value="catalog" />
                	<button type="submit" class="btn-link">${catalog}</button></form></li>
            <c:if test="${user.is_admin == true}">
            	<li><form action="Controller" method="get"><input type="hidden" name="command" value="admin_page" />
                	<button type="submit" class="btn-link">${administration}</button></form></li>
            
            </c:if>
            </ul>
            <ul class="nav navbar-nav navbar-right">
                <li class="dropdown">
                    <a class="dropdown-toggle" role="button" data-toggle="dropdown" href="#"><i class="glyphicon glyphicon-user"></i> ${profile} <span class="caret"></span></a>
                    <ul id="g-account-menu" class="dropdown-menu" role="menu">
                    	<c:choose>
                          <c:when test="${user.loged == true}">
			              	<li><form action="Controller" method="get"><input type="hidden" name="command" value="admin_page" />
                	<button type="submit" class="btn-link">${profile}</button></form></li>
			              </c:when>
			              <c:otherwise>
					...
					<c:if test="${user.loged == true}">
						<li>
							<form action="Controller" method="get">
								<input type="hidden" name="command" value="log_out" />
								<button type="submit" class="btn-link">
									<i class="glyphicon glyphicon-lock"></i>${logout}</button>
							</form>
						</li>
					</c:if>
					<li class="dropdown"><a class="dropdown-toggle" role="button" data-toggle="dropdown" href="#"> ${lang} 
					<span class="caret"></span></a>
						<ul id="g-account-menu" class="dropdown-menu" role="menu">
								<li><form action="Controller" method="post"><input type="hidden" name="command" value="local" />
								<input type="hidden" name="local" value="ru" />
									<button type="submit" class="btn-link">${ru}</button></form></li>
								<li><form action="Controller" method="post"><input type="hidden" name="command" value="local" />
								<input type="hidden" name="local" value="en" />
									<button type="submit" class="btn-link">${en}</button></form></li>
						</ul>
						</li>
				</ul>
        </div>
